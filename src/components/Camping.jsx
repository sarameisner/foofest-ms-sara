...
function Camping({ selectedCampingArea, setCampingArea }) {
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-1 w-[400px] m-auto mt-10 lg:grid-cols-1 gap-5">
          ...
          {campingData.map((area) => (
            <li
              ...
              className={`relative group p-4 rounded-lg cursor-pointer ${selectedCampingArea === area.area ? "border-transparent" : ""}`}
            >
              {/* Border div */}
              <div className="absolute inset-0 z-0 before:content-[''] before:absolute before:-top-1 before:-right-8 before:-z-10 before:w-full before:h-24 before:border-[8px] before:border-[#881523] before:opacity-100 before:rounded-lg transition-all duration-500 ease-out group-hover:top-[1rem] group-hover:left-[1rem]"></div>
    
              {/* Content div with area name and availability, move together on hover */}
              <div className="relative w-full z-10 p-8  group-hover:translate-x-4 group-hover:translate-y-4 transition-all duration-500 ease-out rounded-2xl  flex flex-col justify-center items-center">
  ...
  <h4 className="text-lg">{area.area}</h4>
